...
@title{Interface for Clients of Coloring-Lexer Output}

@defmodule[syntax-color/classified-text]

@history[#:added "1.3"]

@definterface[classified-text<%> ()]{

The @racket[classified-text<%>] interface matches a subset of the
@racket[color:text<%>] interface. It is intended to specify methods
that indentation and expression-navigation functions can use so that
they work either on actual @racket[color:text<%>] objects or in other
